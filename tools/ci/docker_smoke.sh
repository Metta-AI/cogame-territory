...
# Substitute: territory, territory-coworld, 9.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (territory-coworld:ci)
#   SMOKE_SLUG                 game slug                        (territory)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/territory)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/territory-player)
#   SMOKE_MANIFEST             manifest template path           (coworld_manifest_template.json)
#   SMOKE_SEATS                seat-count CROSS-CHECK           (9)
...
image="${1:-${SMOKE_IMAGE:-territory-coworld:ci}}"
slug="${SMOKE_SLUG:-territory}"
